...
- 問題登録:
  ...
  - 文字数は2〜8
  - 追加フォーマット（任意・複数行）で問題文/解答/出題モードを指定可能
    - `【問題文】<text>`（20文字以内、@不可）
    - `【解答】<text>`（20文字以内、@不可。記号や空白を含めて良い）
    - `【出題モード】共通部分/和集合`（未指定時は共通部分）
  - 問題登録成功時に、上記フォーマット案内を2通のメッセージで送信
- 設定:
  - フォントのみ設定可能
  - 設定は `line_settings.json` もしくは `LINE_SETTINGS_FILE_PATH` に保存
- 一括更新:
  - 問題一覧の全文貼り付けで10問分を一括更新
  - 問題文は `@` 行として、解答は `【解答】` 行として含められる
  - 問題一覧の表示では、解答がある場合に `【解答】` 行を追加する
...
- ボットへのメンション:
  ...
  - 解答発表テキストは以下:
    - カスタム解答あり: `合成した文字: <word>\n解答: <custom_answer>`
    - カスタム解答なし: `合成した文字: <word>`
- 他ユーザーへのメンション:
  - `@User <number>.<answer>` で正誤判定を返す
  - カスタム解答があればその文字列で正誤判定を行う
